...
public class Cls_Entrada {
    ...
    public DefaultTableModel getDatosEntradas(){
        try {
            setTitulosEntrada();
            PS = CN.getConnection().prepareCall("{?=call listarEntrada}");
    ...
    public int registrarEntrada(String nfactura, String codigo, Date fecha, int cantidad,String desc){
        int res=0;
        try {
            PS = CN.getConnection().prepareCall("{call add_entrada(?,?,?,?,?,?)}");
